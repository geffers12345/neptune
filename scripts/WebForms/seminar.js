    ...
    (new http).post("seminars.aspx/get", {
        ...
        var items = response.d.map(item => {
            return new Promise(function (resolve, reject) {
                ...
                var req = item.CompanyID == 0 ? "No" : "Yes";
...
$(document).on('click', '#save', function () {
    if (isInputValid()) {
        ...
        var required = $('#required').prop('checked') == true ? 1 : 0;

        (new http).post("seminars.aspx/insert", {
            name: $('#name').val(),
            description: $('#desc').val(),
            required: required,
            notify: parseInt($('#notify').val())
        }).then(function (response) {
...
$(document).on('click', '.view', function () {
    ...
    (new http).post("seminars.aspx/find", {
        id: id
    }).then(function (response) {
        ...
        $('#notify-view').val(item.Notify);

        if (item.CompanyID == 1) {
            $('#required-view').prop('checked', true);
        } else {
            $('#required-view').prop('checked', false);
        }
...
$(document).on('click', '.edit', function () {
    ...
    (new http).post("seminars.aspx/find", {
        ...
        $('#notify-edit').val(item.Notify);

        if (item.CompanyID == 1) {
            $('#required-edit').prop('checked', true);
        } else {
            $('#required-edit').prop('checked', false);
        }
...
$(document).on('click', '#saveChanges', function () {
    if (isInputEditValid()) {
        ...
        var required = $('#required-edit').prop('checked') == true ? 1 : 0;

        (new http).post("seminars.aspx/update", {
            id: id,
            name: $('#name-edit').val(),
            description: $('#desc-edit').val(),
            required: required,
            notify: parseInt($('#notify-edit').val())
        }).then(function (response) {
...
$(document).on('click', '.remove', function () {
    ...
    swal({
        ...
        if (isConfirm.value == true) {
            (new http).post("seminars.aspx/delete", {
                id: id,
                name: name
            }).then(function (response) {
